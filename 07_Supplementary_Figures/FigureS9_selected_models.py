...
import FigureS9_plotting_utils as plotting
plotting.PROJECT_ROOT = DATA_ROOT / '07_Model_Scores'
plotting.LABELS_PATH = DATA_ROOT / '00_Shared_Data_and_Code/Data/CPGEA-TCGA 20230106 OK.csv'

from FigureS9_plotting_utils import (
    RECALL_GRID,
    ROC_GRID,
    SCRIPT_DIR,
    SEED,
...
def plot_overlay_row(
    ...
    roc_axis, pr_axis, dca_axis, calibration_axis = axes
    calibration_axis.plot([0, 1], [0, 1], linestyle="--", color="#8A8A8A", linewidth=1.0)
    roc_axis.plot(
        [0, 1],
        [0, 1],
        linestyle=(0, (4, 3)),
    ...
    for evaluation_index, (
        ...
        key = (specification.code, short_label)
        if key not in CURVE_SUMMARIES:
            CURVE_SUMMARIES[key] = bootstrap_summary(
                y_true,
                score,
                np.random.default_rng(SEED + model_index * 10 + evaluation_index),
            )
        summary = CURVE_SUMMARIES[key]
        ...
        n_bins = min(5 if len(data) >= 100 else 4 if len(data) >= 50 else 3,
                     len(np.unique(score)))
        groups = np.array_split(np.argsort(score, kind="stable"), n_bins)
        predicted_means = [float(score[index].mean()) for index in groups]
        observed_rates = [float(y_true[index].mean()) for index in groups]
        calibration_axis.plot(predicted_means, observed_rates, color=colour,
                              linestyle={"All": "-", "Training": "--", "Test": ":"}[short_label],
                              marker="o", markersize=2.5, linewidth=1.5)
        for bin_number, indices in enumerate(groups, 1):
            CALIBRATION_ROWS.append({
                "modality": specification.code, "selected_model": specification.model,
                "evaluation_set": evaluation_label, "bin": bin_number,
                "N": len(indices), "N_HRD": int(y_true[indices].sum()),
                "mean_predicted": float(score[indices].mean()),
    ...
    calibration_axis.set(xlim=(-0.02, 1.02), ylim=(-0.02, 1.02),
                         xlabel="Mean predicted probability", ylabel="Observed HRD proportion")
    for axis in (roc_axis, pr_axis, dca_axis, calibration_axis):
        style_axis(axis)
...
def create_overlay_figure(
    ...
    *,
    ...
    for column, column_title in enumerate(
        ("ROC", "Precision-recall", "Decision curve", "Calibration")
    ):
    ...
    panel_labels = [chr(65 + SELECTED_MODELS.index(spec)) for spec in specifications]
    ...
    for panel_index, axis in enumerate(axes[:, 0]):
        axis.text(
            -0.17,
            1.08,
...
save_figure(
    overview,
    OUTPUT_DIR / "Figure_S9_Selected_Models_7x4_R1_recalculated",
)
plt.close(overview)

if EXPORT_DATA:
    pd.DataFrame(metrics).to_csv(OUTPUT_DIR / "Figure_S9_Metrics_R1_recalculated.csv", index=False)
    pd.DataFrame(CALIBRATION_ROWS).to_csv(OUTPUT_DIR / "Figure_S9_Calibration_Points_R1_recalculated.csv", index=False)
for row in metrics:
    print(
